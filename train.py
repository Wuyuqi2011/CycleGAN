...
from models import GeneratorL2H, GeneratorH2L, DiscriminatorL, DiscriminatorH
...
parser.add_argument('--size_LR', type=int, default=128, help='size of the data crop (squared assumed)')
parser.add_argument('--size_HR', type=int, default=1024, help='size of the data crop (squared assumed)')
...
# Networks
netG_A2B = GeneratorL2H(opt.input_nc, opt.output_nc)
netG_B2A = GeneratorH2L(opt.output_nc, opt.input_nc)
netD_A = DiscriminatorL(opt.input_nc)
netD_B = DiscriminatorH(opt.output_nc)
...
input_B = Tensor(opt.batch_size, opt.output_nc, opt.size_HR, opt.size_HR)
...
dataloader = DataLoader(ImageDataset(opt.dataroot, transform=transform, unaligned=False, mode='train'),
...
if __name__ == '__main__':
    for epoch in range(opt.epoch, opt.n_epochs):
        for i, batch in enumerate(dataloader):
            ...
            same_B = netG_A2B(netG_B2A(real_B))
            loss_identity_B = criterion_identity(same_B, real_B) * 10.0
            # G_B2A(A) should equal A if real A is fed
            same_A = netG_B2A(netG_A2B(real_A))
            ...
            logger.log(
                losses={'loss_G': loss_G.item(), 'loss_G_identity': (loss_identity_A.item() + loss_identity_B.item()),
